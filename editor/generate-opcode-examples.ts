...
export function run(inFile: string, outFile: string) {
  ...
  console.log(`Generating opcode examples from ${inFile}`);
  ...
  function makeLine(command: Command) {
    ...
    if (command.operator) {
      const cb = (param: Param) =>
        braceify(stringifyTypeAndSource(param), '[]');
  ...
  function stringifyWithColon(p: Param) {
    return [
      [stringifySource(p.source), p.name].filter(Boolean).join(' '),
      p.type,
    ]
      .filter(Boolean)
      .join(': ');
